...
public:
	...
	sc_in <int> req_count_to_llc_l2;
	sc_out <int> req_count_from_llc_l2;
	sc_in <bool> ex_to_dir_l2;
	sc_in <bool> ce_to_dir_l2;
	sc_in <bool> rd_wr_to_dir_l2;
	sc_out <bool> ack_from_dir_l2;
	sc_out <bool> ack_from_llc_l2;
	sc_in <int> address_to_llc_l2;
	sc_out <int> address_from_llc_l2;
	sc_in <bool> address_to_llc_en_l2; // l2 can start pass data to llc with address , if LLC is not present direct to MEM.
	sc_port<write_read_if> data_to_llc_l2;
	sc_port<write_read_if> data_from_llc_l2;
...
// l2
	...
	CM_LSU(sc_module_name name, int id): sc_module(name), inode(id),trans_ack_from_mem_llc(0),addr_en_from_l2_to_llc(0),ack_from_llc_dir(0),ack_from_dir_l2(0)
	{
...
private:
	...
	int inode;
	queue <NoCCommPacket>  local_l2Q, local_dirQ, local_llcQ;
};
