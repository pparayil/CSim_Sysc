...
// l2
	sc_in <int> addr_f_l2;
	sc_out <int> addr_t_l2;
	sc_in <int> req_count_f_l2;
	sc_out <int> req_count_t_l2;
	sc_in <bool> addr_en_frm_l2; 
...
// mem
	...
	SC_HAS_PROCESS(LLC);
	LLC(sc_module_name name, int id, bool instantiate):sc_module(name),inode(id), ack_to_dir(0x0),ack_to_l2(0),nodeaddr_to_lru(0x0),addr_en_to_mem(0),r_w_to_mem(0),addr_to_mem(0x0){
...
private:
	...
	*/
	...
	void ack_to_npc_dir(unsigned int address, datatype *data, int req_count, bool we, unsigned int npc_node, unsigned int dir_node, bool dirack_en);
	void rdwr_to_mem ( unsigned int address, datatype *data, int req_count, bool we, unsigned int npc_node, unsigned int dir_node, unsigned int way, bool dirack_en);
