...
class ReqFormat {
public : 
enum ReQType {RD, WR, INV, FLUSH, RDX,  ACK_SMALL, ACK_FULL, WR_BACK};
	datatype data;
	int address;
	bool we;
	bool ex;
	...
	linked = false;
	 ...
	 bool operator!=(const ReqFormat& other){
		if((this->address == other.address) && (this->req_counter == other.req_counter) && (this->type == other.type))
			return false;
		else 
			return true;
	 }
...
private:	
	...
	*/
	...
	bool rd_rdx_to_dir (int address, bool Exclusive, bool refill,int req_count);
	void write_to_llc ( int address, datatype * DATA, bool evict, int req_count, int way, bool EN_req_to_directory);
	bool invalidating_cl (int address ,int way,int req_count, unsigned int type,ReqFormat::ReQType tp);
	char find_free_way (int address,int req_count,bool *wb_enable, bool *status);
