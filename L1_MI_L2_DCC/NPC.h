...
class ReqFormat {
public : 
enum ReQType {RD, WR, INV, FLUSH, RDX, FWD_RD, SPILL_RD, ACK_SMALL, ACK_FULL, FWD_RD_ACK, WR_BACK, SPILL_DATA_DCC_ACK, REPL_REQ_DCC_ACK, SPILL_DATA_DCC, DCC_REPLACEMENT};
	datatype data;
	int address;
	bool we;
	bool fwd;
...
// L1 side
	...
	/* 4 bit state 
		...
		// transition states: 
		1000: F to I,
		1001: M to I DCC replacement	
		1010: F to I singlet DCC replacement,		
		1011: Shared to M ,
		1100 : INV - SHARED (RD) ,
		...
		*/
	enum NPC_STATES {INVALID,SHARED,MODIFIED,FORWARD,FORWARD_SNG,S_SP,M_SP,F_M, F_I, SNG_DCC_TRANS,F_I_SNG, S_M, I_S, S_I, I_M, M_I_WB};
private:	
	...
	*/
	...
	void write_to_l3 ( int address, datatype * DATA, bool evict, int req_count, int way, bool EN_req_to_directory, unsigned int ack_receiver, bool dcc, bool dirty);
	bool invalidating_cl (int address ,int way,int req_count, unsigned int type,ReqFormat::ReQType tp, unsigned int redirect_node);
	char find_free_way (int address,int req_count,int *prev_state, bool *status, bool dcc_reception);
	...
	void req_to_DIR (bool evict,bool rw,bool ex,bool ce, unsigned int address,unsigned int req_count, bool dcc_en, bool inv_ack);
