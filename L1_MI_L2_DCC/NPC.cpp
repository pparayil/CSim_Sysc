...
void NPC::req_to_DIR (bool evict,bool rw,bool ex,bool ce, unsigned int address,unsigned int req_count, bool dcc_en, bool inv_ack) {
...
void NPC::write_to_l3 ( int address, datatype * DATA, bool evict, int req_count, int way, bool EN_req_to_directory, unsigned int ack_receiver, bool dcc, bool dirty){
	//cout<<"NPC::"<<__func__<<" : "<< __LINE__<<" evict:"<<(unsigned int)evict<<endl;		
	ReqFormat ReQ;
	if(EN_req_to_directory == true)
	{
		if(dcc == true){
			ReQ.type = ReqFormat::DCC_REPLACEMENT;
			unsigned int dcc_node = rand () % (TTL_NO_NPC); 
			/*if directory is not aware of the data passing from npc-dcc, there cud b coherency issues*/
...
char NPC::find_free_way (int address,int req_count,int *prev_state, bool *status, bool dcc_reception) {
		...
		for(; j<NPC_WAYS; j++)
		{
			if (NPCtag[2][set][j] == LRUvalue)
			{
				...
				*prev_state = NPCtag[1][set][j];
				...
				{ // not singlet
					cout<<"NPC::"<<inode << " : "<<__func__<<" : "<< __LINE__<<" In state:"<<(unsigned int) NPCtag[1][set][j] <<", address:"<<hex<<existingaddr<<dec<<endl;		
				...
				{
					cout<<"NPC::"<<inode << " : "<<__func__<<" : "<< __LINE__<<" In Modified Spilled state"<<endl;
				...
				else if ((NPCtag[1][set][j] == MODIFIED) || (NPCtag[1][set][j] == FORWARD_SNG)) // Modified / fwd singlet
				{
					...
					if(dcc_reception == false){
						sng_way_list_counter ++;
						sng_way_list[(sng_way_list_counter- 1)] = j;
						/*if(NPCtag[1][set][j] == MODIFIED)
							write_to_l3(existingaddr,&NPCMem[j][set], true,req_count,j, true, inode, true, true); // eviction DCC replacement
						else 
							write_to_l3(existingaddr,&NPCMem[j][set], true,req_count,j, true, inode, true, false); 
						NPCtag[1][set][j] = SNG_DCC_TRANS; // M/F_SNG to I as dcc replacement
						//INV_L1(existingaddr,req_count); 
						cout<<"NPC::"<<inode << " : "<<__func__<<" : "<< __LINE__<<" SNG DCC on move, address:"<<hex<<existingaddr<<dec<<endl;
						found = true;*/
					} else {
						continue;
					}
...
bool NPC::invalidating_cl (int address ,int way, int req_count, unsigned int state, ReqFormat::ReQType tp, unsigned int redirect_node)
{
	...
	else if (state > M_SP) //transition state
	{
		...
		} else
		{
			...
			do {
				front = BlockedQ.front();			
				if(((front.address >> CL_SIZE) == (address >> CL_SIZE))  && ((front.type != ReqFormat::WR_BACK) && (front.type != ReqFormat::DCC_REPLACEMENT) && (front.type != ReqFormat::INV) && (front.type != ReqFormat::FLUSH)))
				{
				...
				else if (((front.address >> CL_SIZE) == (address >> CL_SIZE))  && ((front.type == ReqFormat::WR_BACK) || (front.type == ReqFormat::DCC_REPLACEMENT) || (front.type == ReqFormat::FWD_RD)) && (front.linked == true)) 
				{
					if(back != front)
						BlockedQ.pop();
					else {
					...
					if((fronttemp.type == ReqFormat::WR) || (fronttemp.type == ReqFormat::RD)){
						...
						} else if (front.type == ReqFormat::DCC_REPLACEMENT) {
		...
		case ACK_ANY:{
					...
					if((ack_from_dir == true)) {
						if (replace_req_dcc_ack_from_dir == true) {
							ReQ3.type = ReqFormat::REPL_REQ_DCC_ACK;
...
void NPC::npc_main_thread(void){
	...
	while (1) {	
		...
		switch (progrs_ReQ.type) {
			...
			case ReqFormat::SPILL_DATA_DCC_ACK: // npc or memory
			case ReqFormat::REPL_REQ_DCC_ACK: // Dir
						...
						} else
						{
							...
							do {
								...
								if(((front.address >> CL_SIZE) == (progrs_ReQ.address >> CL_SIZE)) && (front.req_counter == progrs_ReQ.req_counter))
								{
									...
									if (((front.type == ReqFormat::DCC_REPLACEMENT) && (front.linked == true)) && ((progrs_ReQ.type == ReqFormat::REPL_REQ_DCC_ACK)) && ((state == SNG_DCC_TRANS))){
										// do a DCC write to the got nodeaddress
										if(progrs_ReQ.ex == true){ // dcc transfer enabled 
									...
									else if (((front.type == ReqFormat::DCC_REPLACEMENT)) && ((progrs_ReQ.type == ReqFormat::SPILL_DATA_DCC_ACK) || (progrs_ReQ.type == ReqFormat::ACK_FULL)) && ((state == SNG_DCC_TRANS))){
										BlockedQ.pop(); // this cant be the back in the queue
										front = BlockedQ.front(); //actual req received RD or WR
										cout << inode <<" : @NPC::"<<__func__ <<" : "<< __LINE__<< "ACK type5"<<endl;
										if((front.type == ReqFormat::RD)) {
											...
											if((state == F_I) ){
												if((front.linked == true) && (front.type == ReqFormat::FWD_RD)){
													...
													if((front.type == ReqFormat::RD)) {
														...
														BlockedQ.pop();
														BlockedQ.push(front);
													} else if (front.type == ReqFormat::SPILL_DATA_DCC) {
					...
					// address in transition state, push the request to back 
						...
						if(status == false)
							...
							if((prev_state == MODIFIED) || (prev_state == FORWARD_SNG) || (prev_state == S_SP) || (prev_state == M_SP) || (prev_state == FORWARD)){
								if((prev_state == MODIFIED) || (prev_state == FORWARD_SNG)) { // require DCC spill
									//wait for ack from DCC or L3/MEM then proceed with request
									cout<<"NPC::"<<inode<<" : "<<__func__<<" : "<< __LINE__<<" step 5DCC"<<endl;
								} 
			...
			case ReqFormat::SPILL_DATA_DCC: {
					...
					if(status == false) {
						...
						if((prev_state == M_SP) || (prev_state == FORWARD) || (prev_state == S_SP)) {
							...
							ReQ.ex = progrs_ReQ.ex;			
							ReQ.address = progrs_ReQ.address;
							ReQ.type = ReqFormat::SPILL_DATA_DCC;
							ReQ.we = progrs_ReQ.we;
							ReQ.way = way;
							ReQ.data = progrs_ReQ.data;
							ReQ.linked = false;
