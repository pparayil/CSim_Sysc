...
void LLC::rdwr_to_mem ( unsigned int address, datatype *data, int req_count, bool we, unsigned int npc_node, unsigned int dir_node, unsigned int way, bool dirack_en, bool mode_pri_remote) {
	...
	if((we == true) && (data == NULL))
		...
		ReQ.mode_pri_remote = mode_pri_remote;
		BlockedQ.push(ReQ);
	} else {
		ReqFormat_LLC ReQ;
...
void LLC::ack_to_npc_dir(unsigned int address, datatype *data, int req_count, bool we, unsigned int npc_node, unsigned int dir_node, bool dirack_en, bool mode_pri_remote){
	cout<<"LLC::"<<inode<<", "<<__func__<<" : "<< __LINE__<<endl;
	...
	addr_t_npc = address;
	req_count_t_npc = req_count;
	ack_to_npc = true;
	ack_rw_to_npc = we;
	mode_pri_remote_to_npc = mode_pri_remote; 
	wait();
	ack_to_npc = false;
	mode_pri_remote_to_npc = false; //false = priv, 	true = remote
	wait();
	if(dirack_en == true) {
		ack_to_dir = true;
		addr_t_dir = address;
		...
		{			
			if(LLCtag[coop_sect][2][i][set] < (c+1)){
				if(LLCtag[coop_sect][1][i][set] == (unsigned int) ReqFormat_LLC::REG_M) { // need to writeback
					...
					rdwr_to_mem(temp_address,&LLCMem[coop_sect][i][set],req_count, true, 0x0,0x0, i, false, false);
...
void LLC::LLC_request_collector(void){
	...
	while (1)
		{		
		if(addr_en_frm_npc == true){
			ReqData_LLC ReQ1;
			ReQ1.npc_node = node_address_from_lru;
			ReQ1.address = addr_f_npc;
			ReQ1.req_count = req_count_f_npc;
			data_in->read_data(&ReQ1.data);
			ReqQ_data.push(ReQ1);
			cout<<"LLC::"<<inode<<", "<<__func__<<" : "<< __LINE__<<" for address: "<<hex<< addr_f_npc << " from "<<node_address_from_lru<<dec<<" DataQsize:"<< ReqQ_data.size()<< endl;	
		...
		if (ce_ == true)
		{
			...
			ReQ.mode_pri_remote = mode_pri_remote_f_dir;
			if(rd_wr_ == true)
			{ 					
			ReQ.we = true;					
			...
			cout<<"LLC::"<<inode<<", "<<__func__<<" : "<< __LINE__<<" LLC Req Q size:"<<ReqQ.size()<<" addr:0x"<<hex<<addr_f_dir<<dec<<endl;
			ReqQ.push(ReQ); 
			/*if(inode == 0x0) {
			cout<<endl<<"$$$$$$$$$$$$$$$$$$$$$"<<endl;
			//cout<<"llc: "<<hex<<"0x"<<ReQ.address<<" @node:"<<inode<<" hit : "<<hit<<dec<<endl;
	...
	while (1) {
		...
		switch (ReQ.type) {
					...
					if (hit == false) {
						...
						if(find_way(ReQ.address,&way,ReQ.req_count,&WB_enable) == false){
							...
							if(WB_enable == false){
								...
								rdwr_to_mem(ReQ.address,NULL,ReQ.req_count, false,ReQ.npc_node,ReQ.dir_node,way,ReQ.dirack_en, ReQ.mode_pri_remote);	
						...
						if ((hit == true) && (state > (unsigned int)(ReqFormat_LLC::REG_M))) { // trans state
							...
							ack_to_npc_dir(ReQ.address,&LLCMem[coopert_section][way][set],ReQ.req_count,false,ReQ.npc_node,ReQ.dir_node, ReQ.dirack_en, ReQ.mode_pri_remote);
						}
					}
				} break;
			case ReqFormat_LLC::WR:{
				...
				if(find_in_blockedQ(ReQ.address) == true){
					found = false;
				} else if((ReqQ_data.empty() == true) || ((hit == true) && (state > (unsigned int)(ReqFormat_LLC::REG_M)))){ // trans state or no data
					found = false;
				} 
				else {
					//cout<<"LLC::"<<inode<<" : "<<__func__<<" : "<< __LINE__<<" WR npc data received for address: 0x"<<hex<< ReQ.address << " from npc: "<<ReQ.npc_node<<dec<<endl;
							...
							if (hit == false){
								if(find_way(ReQ.address,&way,ReQ.req_count,&WB_enable) == false){
									...
									if(WB_enable == false){									
										...
										LLCtag[coopert_section][2][way][set] = (LLC_WAYS -1); //0x7;
										ack_to_npc_dir(ReQ.address,NULL,ReQ.req_count,true,ReQ.npc_node,ReQ.dir_node, ReQ.dirack_en,ReQ.mode_pri_remote);
									}
									else {
										ReQ.we = true;
										ReQ.way = way;
						...
						if(((ReQ.address) == (front.address)) && (ReQ.req_count == front.req_count)&& ((front.type == (unsigned int)ReqFormat_LLC::WR_BACK) || (front.type == (unsigned int)ReqFormat_LLC::RD))){
							...
							if((ReQ.we == true) && (front.type == (unsigned int)ReqFormat_LLC::WR_BACK) && (state == (unsigned int)(ReqFormat_LLC::TRANS_MI))){
								...
								if(front.type == (unsigned int)ReqFormat_LLC::RD){
									...
									rdwr_to_mem(front.address,NULL,front.req_count, false,front.npc_node,front.dir_node,front.way, front.dirack_en, front.mode_pri_remote);
									if(back != front)
										BlockedQ.pop();
									else{
										BlockedQ.pop();
									...
									ack_to_npc_dir(front.address,NULL,front.req_count,true,front.npc_node,front.dir_node,front.dirack_en,front.mode_pri_remote);												
									if(back != front)
										BlockedQ.pop();
									else{
										BlockedQ.pop();
								...
								ack_to_npc_dir(front.address,&LLCMem[coopert_section][front.way][set],front.req_count,false,front.npc_node,front.dir_node, front.dirack_en, front.mode_pri_remote);												
								if(back != front)
									BlockedQ.pop();
								else{
									BlockedQ.pop();
