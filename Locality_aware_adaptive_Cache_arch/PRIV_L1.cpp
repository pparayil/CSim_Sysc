...
bool PRIV_L1::invalidating_cl (int address ,int way, int req_count, unsigned int state, ReqFormat_PrivL1::ReQType tp, bool hit)
{
	int set = (address >> CL_SIZE) & ((unsigned int)PRIV_L1_SET_COUNT-1); 
	int tag = (address) >> (PRIV_L1_SET_COUNT_2 + CL_SIZE);
	...
	else  //transition state
	{
		...
		{
			if(hit == true) PRIV_L1tag[1][set][way] = 0x0;
	...
	while (i>0)
	{
		...
		case ACK_ANY:{
					...
					if (ack_from_l2 == true) {
						...
						ReQ3.we = ack_we_l2;
						ReQ3.rm = mode_pri_remote_from_l2;
						if(!(ack_we_l2))
	...
	while (1) {	
		...
		switch (progrs_ReQ.type) {
			case ReqFormat_PrivL1::INV: {
					bool ret = false;
					if((hit == true)) 
						invalidating_cl(progrs_ReQ.address,way,progrs_ReQ.req_count, state,progrs_ReQ.type, true);
					else {
						if (find_in_blockedQ(progrs_ReQ.address) == true){
							invalidating_cl(progrs_ReQ.address,0,progrs_ReQ.req_count,0x3,progrs_ReQ.type, false);
						}
					}
				} break;
			case ReqFormat_PrivL1::ACK_L2:{
					...
					/*if ((hit == false) || (state == 0x1)) {
						...
						if(BlockedQ.empty() == true) 
						{
							cout << inode <<" : @PRIV_L1::"<<__func__ <<" : "<< __LINE__<<"ACK received for non-blocked req: "<<"address:"<<hex<<progrs_ReQ.address<<dec<< endl;
						} else
						{
							ReqFormat_PrivL1 front;
							ReqFormat_PrivL1 back = BlockedQ.back();
								...
								if(((front.address >> CL_SIZE) == (progrs_ReQ.address >> CL_SIZE)) && (front.req_count == progrs_ReQ.req_count))
								{
									...
									*/									
									if ((((front.type == ReqFormat_PrivL1::RD) && (progrs_ReQ.we == false) /*&& (state == 0x2)*/) || ((front.type == ReqFormat_PrivL1::WR) && (progrs_ReQ.we == true) /*&& (state == 0x3)*/))){ 
										...
										if(progrs_ReQ.rm == true) {
											if(hit == true) {
												PRIV_L1tag[1][set][way] = 0x0;
											}
											if((front.type == ReqFormat_PrivL1::RD)){
												DATA_DB_OUT->write_data(&progrs_ReQ.data);
												WE_DB_OUT = false;
											} else {
												WE_DB_OUT = true;
											}
											...
											if(status == false) {
												...
												if((front.type == ReqFormat_PrivL1::RD)){											
													PRIV_L1Mem[way][set] = progrs_ReQ.data;
													DATA_DB_OUT->write_data(&PRIV_L1Mem[way][set]);
													WE_DB_OUT = false;
												} else {
													PRIV_L1Mem[way][set] = front.data;
													WE_DB_OUT = true;
												}										
												for(char j=0; j<PRIV_L1_WAYS; j++)
												{
												...
												PRIV_L1tag[2][set][way] = PRIV_L1_WAYS-1;
												PRIV_L1tag[1][set][way] = 0x1;
												PRIV_L1tag[0][set][way] = (progrs_ReQ.address) >> (PRIV_L1_SET_COUNT_2 + CL_SIZE);
												REQ_COUNT_OUT = progrs_ReQ.req_count;
												ADDRESS_DB_OUT = progrs_ReQ.address;
												STATUS_DB = true;
												ACK_DB = true;													
									...
									{
										cout << inode <<" : @PRIV_L1::"<<__func__ <<" : "<< __LINE__<< " Error? "<<hex<<progrs_ReQ.address<<dec<<" for type:" <<front.type<<" @state:"<<state<<endl;
										BlockedQ.pop();
										BlockedQ.push(front);
									}
								} 							
